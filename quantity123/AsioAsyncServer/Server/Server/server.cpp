...
CClient::CClient(tcp::socket aSocket)
    : mClientSocket(move(aSocket)), mClientStrand(aSocket.), mClientName(""), mIsLogin(false)
{
}
...
void CListen::acceptClientConnect()
{
    auto acceptClientConnectCallBack = [this](boost::system::error_code ec)
    {
        if (!ec)
        {
            CClientPtr client = make_shared<CClient>(move(mListenSocket));
...
CListen::CListen(io_service& ioService, const tcp::endpoint& endPoint)
    : mListenSocket(ioService), mAcceptor(ioService, endPoint), mListenStrand(ioService), mTestClientOnlineTimer(ioService, chrono::seconds(5))
{
    acceptClientConnect();
    auto lbdTestClientOnline = [this](const boost::system::error_code& ec)
    {
